...
def borrow_book(request, book_id):
    ...
    return render(request, 'home/borrow_book.html', {'book': book})
...
def borrowed_books_view(request):
    ...
    if user.is_staff or getattr(user, 'role', None) in ['admin', 'librarian']:
        ...
        borrowed_books = Borrow.objects.select_related('book__category', 'user').order_by('book__category')
    else:
        # Students see only their own borrowed books, ordered by category
        borrowed_books = Borrow.objects.select_related('book__category', 'user').filter(user=user).order_by('book__category')
...
def approve_borrowed_books(request):
    ...
    return render(request, 'admin/approve_borrowed_books.html', {'pending_borrows': pending_borrows})
